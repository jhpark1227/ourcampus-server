...
public record CreateFacilityWebRequest(
        String name,
        String description,
        String purpose,
        String equipment,
    ...
    public CreateFacilityRequest toDto() {
        return new CreateFacilityRequest(
                ...
                operationTimes()
                        ...
                        .map(o -> {
                            ...
                            return new CreateFacilityRequest.OperationTimeRequest(o.name(), startTimeMinuteOffset, endTimeMinuteOffset);
                        })
                        .toList(),
                images(),
                new CreateFacilityRequest.ReservationPolicyRequest(reservationPolicy.reservable, reservationPolicy.availableTimes.stream()
                        .map(o -> {
                            String[] parts = o.startTime().split(":");
                            int startTimeMinuteOffset = Integer.parseInt(parts[0]) * 60 + Integer.parseInt(parts[1]);
                            parts = o.endTime().split(":");
                            int endTimeMinuteOffset = Integer.parseInt(parts[0]) * 60 + Integer.parseInt(parts[1]);
                            return new CreateFacilityRequest.AvailableTimeRequest(startTimeMinuteOffset, endTimeMinuteOffset);
                        })
                        .toList()),
                buildingId,
                themeIds
